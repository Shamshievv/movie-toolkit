...
export const getPopular = (page:number,language:any) => {
    return async (dispatch:AppDispatch) => {
       try {
           ...
           const responsive = await axios(`https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=${language}&page=${page}&sort_by=vote_average.desc&without_genres=99,10755&vote_count.gte=200&api_key=${API_KEY}`)
